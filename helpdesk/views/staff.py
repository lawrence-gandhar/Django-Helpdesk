...
import helpdesk.helper as helper
...
def dashboard(request):
    ...
    # code added by Lawrence Gandhar (July-2019)
    # code block started - (For Graph Visualizatuion)
    #
    ...
    if request.POST:
        s_year = request.POST['selected_year']
        s_month = request.POST['selected_month']		
    ...
    pie_chart_data = helper.pie_chart_data(request, s_year, s_month)

    line_chart_data = helper.line_chart_data(request, s_year, s_month)
    ...
    tickets_closed_resolved = Ticket.objects.select_related('queue').filter(
        assigned_to=request.user,
        status__in=[Ticket.CLOSED_STATUS, Ticket.RESOLVED_STATUS])[:20] 
    ...
    return render(request, 'helpdesk/dashboard.html', {
        ...
        'years_range': [x for x in range(timezone.now().year, 2016, -1)],
...
#
# View written by Lawrence Gandhar
# 28th June 2019 
#
@staff_member_required
def ticket_list(request):
	...
	records_per_page = request.GET.get('records_per_page', 25)
	show_tickets = request.GET.get('show_tickets',None)	

	user_queues = _get_user_queues(request.user)
	base_tickets = Ticket.objects.filter(queue__in=user_queues)

	if show_tickets == 'taken_over_by_me':
		base_tickets = base_tickets.filter(assigned_to_id = request.user)
	if show_tickets == 'unassigned_tickets':
		base_tickets = base_tickets.filter(assigned_to_id = None)
	if show_tickets == 'open_tickets':
		base_tickets = base_tickets.filter(status = 1)	
	if show_tickets == 'reopened_tickets':
		base_tickets = base_tickets.filter(status = 2)
	if show_tickets == 'resolved_tickets':
		base_tickets = base_tickets.filter(status = 3)
	if show_tickets == 'closed_tickets':
		base_tickets = base_tickets.filter(status = 4)
	if show_tickets == 'duplicate_tickets':
		base_tickets = base_tickets.filter(status = 5)
	if show_tickets == 'onhold_tickets':
		base_tickets = base_tickets.filter(status = 6)
	
	base_tickets = base_tickets.select_related('queue',).order_by("-created")
	
	paginator = Paginator(base_tickets, records_per_page) 
...
def ticket_list(request):
    ...
    return render(request, 'helpdesk/ticket_list.html', dict(
        context,
        tickets=ticket_qs,
        default_tickets_per_page=request.user.usersettings_helpdesk.settings.get('tickets_per_page') or 5,
        user_choices=User.objects.filter(is_active=True, is_staff=True),
...
def create_ticket(request):
    ...
    return render(request, 'helpdesk/create_ticket.html', {
		'form': form, 
		'heading_title':'Create Ticket',
		'recent_tickets':Ticket.objects.filter(queue__in=_get_user_queues(request.user)).order_by('-created')[:15]
	})
...
def hold_ticket(request, ticket_id, unhold=False):
    ...
    if unhold:
        ...
        ticket.status = Ticket.ONHOLD_STATUS	
    ...
    # Status Update Added by Lawrence Gandhar
